...
class App::Build < ApplicationRecord
  ...
  def update_available?
    return true unless self.wrapper_version == "Wrapper::#{self.platform.camelize}".constantize.new(beta: self.beta).version
    false
  end
  def build
    if self.can_build?
  ...
  def set_wrapper_version
    self.wrapper_version = "Wrapper::#{self.platform.camelize}".constantize.new(beta: self.beta).version
  end
